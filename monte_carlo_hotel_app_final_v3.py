...
# Função para gerar PDF com os resultados (versão aprimorada)
def gerar_pdf_relatorio(resultados, instancias_por_comodo, num_simulacoes, tempo_total, imagens_graficos=None, comodos_config_data=None, comodos_originais=None):
    """Gera um relatório PDF técnico aprimorado com os resultados da simulação"""
    
    # Template HTML para o relatório (versão aprimorada)
    html_template = """
    <!DOCTYPE html>
    <html>
    <head>
        <meta charset="UTF-8">
        <title>Relatório de Simulação Monte Carlo - Análise de Carga Elétrica</title>
        <style>
            body {
    ...
    <body>
        <div class="header">
            <h1>RELATÓRIO TÉCNICO DE SIMULAÇÃO MONTE CARLO</h1>
            <h2>Análise de Carga Elétrica para Dimensionamento de Infraestrutura Hoteleira</h2>
            <p><strong>Data de geração:</strong> {{ data_geracao }}</p>
            <p><strong>Sistema:</strong> Simulação Monte Carlo com Instâncias Individualizadas</p>
        </div>

        <div class="developer-credits">
            <h4>💻 Sistema Desenvolvido por Matheus Vianna</h4>
            <p><strong>Engenheiro Especialista em Simulação Monte Carlo</strong></p>
            <p>Website: <a href="https://matheusvianna.com" target="_blank">matheusvianna.com</a></p>
            <p>Sistema avançado para análise de carga elétrica utilizando técnicas de simulação Monte Carlo</p>
        </div>

        <div class="section">
            <h2>1. METODOLOGIA E FUNDAMENTOS TEÓRICOS</h2>
            
            <div class="metodologia-box">
                <h3>1.1 Simulação Monte Carlo Aplicada</h3>
                <p>A simulação Monte Carlo é uma técnica estatística que utiliza amostragem aleatória repetitiva para obter resultados numéricos de problemas complexos. No contexto deste estudo, a metodologia foi aplicada para modelar o comportamento estocástico da demanda elétrica em estabelecimentos hoteleiros, considerando a variabilidade natural do uso de equipamentos pelos hóspedes.</p>
                
                <p>O sistema implementado realiza {{ num_simulacoes }} simulações independentes, cada uma representando um cenário possível de operação do hotel durante um período de {{ tempo_horas }} horas. Esta abordagem permite capturar a incerteza inerente ao comportamento dos usuários e fornecer estatísticas robustas para o dimensionamento da infraestrutura elétrica.</p>
            </div>
            
            <h3>1.2 Modelagem de Instâncias Individualizadas</h3>
            <p>Uma característica fundamental desta simulação é o tratamento individualizado de cada unidade habitacional. Quando o hotel possui múltiplas unidades do mesmo tipo (por exemplo, 28 quartos padrão), cada uma é modelada como uma entidade independente com seu próprio comportamento aleatório. Esta abordagem é crucial para capturar adequadamente o fator de diversidade, que representa a probabilidade de que nem todos os equipamentos operem simultaneamente em sua capacidade máxima.</p>
            
            <h3>1.3 Parâmetros de Entrada e Configuração</h3>
            <div class="parametros-lista">
                <p><strong>Número total de simulações realizadas:</strong> {{ num_simulacoes }}</p>
                <p><strong>Período de análise por simulação:</strong> {{ tempo_total }} minutos ({{ tempo_horas }} horas)</p>
                <p><strong>Tipos de cômodos analisados:</strong> {{ total_comodos }}</p>
                <p><strong>Total de instâncias simuladas:</strong> {{ total_instancias }}</p>
                <p><strong>Resolução temporal:</strong> 1 minuto</p>
                <p><strong>Método de amostragem:</strong> Pseudo-aleatório com distribuições específicas por equipamento</p>
            </div>
        </div>

        <div class="section">
            <h2>2. RESULTADOS ESTATÍSTICOS E ANÁLISE DE DEMANDA</h2>
            
            <div class="metrics-grid">
                <div class="metric-card">
                    <div class="metric-value">{{ pico_medio }} W</div>
                    <div class="metric-label">Demanda Média Máxima</div>
                </div>
                <div class="metric-card">
                    <div class="metric-value">{{ pico_max }} W</div>
                    <div class="metric-label">Demanda Máxima Absoluta</div>
                </div>
                <div class="metric-card">
                    <div class="metric-value">{{ pico_min }} W</div>
                    <div class="metric-label">Demanda Mínima Observada</div>
                </div>
                <div class="metric-card">
                    <div class="metric-value">{{ pico_95 }} W</div>
                    <div class="metric-label">Percentil 95 (P95)</div>
                </div>
            </div>
            
            <div class="destaque">
                <h4>Interpretação dos Resultados Estatísticos</h4>
                <p>O <strong>Percentil 95 (P95)</strong> de {{ pico_95 }} W representa o valor de demanda que é excedido em apenas 5% dos cenários simulados. Este valor é amplamente reconhecido na engenharia elétrica como referência para dimensionamento de sistemas, pois oferece um equilíbrio adequado entre segurança operacional e viabilidade econômica.</p>
                
                <p>A <strong>Demanda Média Máxima</strong> de {{ pico_medio }} W indica o valor esperado da demanda de pico, enquanto a <strong>Demanda Máxima Absoluta</strong> de {{ pico_max }} W representa o cenário mais crítico observado nas simulações, que possui probabilidade muito baixa de ocorrência.</p>
            </div>
        </div>

        <div class="section">
            <h2>3. CONFIGURAÇÃO DETALHADA DOS CÔMODOS</h2>
            
            <h3>3.1 Resumo por Tipo de Cômodo</h3>
            <table class="table">
                <thead>
                    <tr>
                        <th>Tipo de Cômodo</th>
                        <th>Número de Instâncias</th>
                        <th>Demanda Estimada por Instância (W)</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><strong>{{ comodo }}</strong></td>
                        <td style="text-align: center;">{{ instancias }}</td>
                        <td style="text-align: center;">{{ potencias_por_comodo.get(comodo, 'N/A') }}</td>
                        <td style="text-align: center;">{{ "%.1f"|format((instancias / total_instancias * 100)) }}%</td>
                    </tr>
                    {% endfor %}
                </tbody>
            </table>
            ...
            <h3>3.2 Especificação Técnica dos Equipamentos</h3>
            <p>A tabela a seguir apresenta a configuração detalhada de cada equipamento por tipo de cômodo, incluindo características operacionais e parâmetros de simulação utilizados.</p>
            
            {% for comodo_nome, equipamentos_df in comodos_config_data.items() %}
            <h4>{{ comodo_nome }}</h4>
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th>Período de Funcionamento</th>
                        <th>Probabilidade de Uso</th>
                        <th>Fator de Demanda</th>
                    </tr>
                </thead>
                <tbody>
                    {% for row in equipamentos_df %}
                    <tr>
                        <td><strong>{{ row['Equipamento'] }}</strong></td>
                        <td style="text-align: center;">{{ row['Potência'] }}</td>
                        <td style="text-align: center;">{{ row['Quantidade'] }}</td>
                        <td style="text-align: center;">{{ row['Tipo de intervalo']|title }}</td>
                        <td>{{ row['intervalo'] }}</td>
                        <td style="text-align: center;">{{ "%.1f"|format(row['probabilidade'] * 100) }}%</td>
                        <td style="text-align: center;">{{ "%.2f"|format(row['FD']) }}</td>
                    </tr>
                    {% endfor %}
                </tbody>
            </table>
        ...
        {% if imagens_graficos %}
        <div class="section">
            <h2>4. ANÁLISES GRÁFICAS E INTERPRETAÇÕES TÉCNICAS</h2>
            
            <p>As análises gráficas apresentadas a seguir fornecem insights fundamentais sobre o comportamento da demanda elétrica do estabelecimento, permitindo uma compreensão abrangente dos padrões de consumo e suas implicações para o dimensionamento da infraestrutura.</p>
            
            {% for imagem in imagens_graficos %}
            <div class="grafico">
                <div class="grafico-titulo">{{ imagem.titulo }}</div>
                <img src="data:image/png;base64,{{ imagem.base64 }}" alt="{{ imagem.titulo }}">
                <div class="grafico-descricao">
                    <strong>Análise Técnica:</strong> {{ imagem.descricao }}
                    
                    {% if "Distribuição" in imagem.titulo %}
                    <br><br><strong>Aplicação Prática:</strong> Este gráfico é fundamental para determinar a capacidade dos transformadores e sistemas de proteção. A forma da distribuição indica o nível de previsibilidade da demanda: distribuições mais concentradas sugerem comportamento mais previsível, enquanto distribuições mais dispersas indicam maior variabilidade e necessidade de margens de segurança maiores.
                    {% elif "Curva de Duração" in imagem.titulo %}
                    <br><br><strong>Aplicação Prática:</strong> A curva de duração é essencial para análises econômicas de sistemas de geração e para dimensionamento de sistemas de armazenamento de energia. Ela permite identificar por quanto tempo determinados níveis de demanda são mantidos, auxiliando na definição de estratégias de gestão energética e na avaliação da viabilidade de sistemas complementares de geração.
                    {% elif "Perfil de Carga" in imagem.titulo %}
                    <br><br><strong>Aplicação Prática:</strong> O perfil de carga médio é crucial para o planejamento operacional e para a negociação de contratos de fornecimento de energia. Ele revela os horários de maior e menor demanda, permitindo a implementação de estratégias de gestão de demanda e a otimização de tarifas horárias. Também é fundamental para o dimensionamento de sistemas de climatização e ventilação.
                    {% elif "Potência Cumulativa" in imagem.titulo %}
                    <br><br><strong>Aplicação Prática:</strong> Esta análise permite identificar quais tipos de cômodos contribuem mais significativamente para a demanda total, orientando decisões sobre eficiência energética e priorização de investimentos em equipamentos mais eficientes. É particularmente útil para estratégias de retrofit e para o planejamento de expansões futuras do estabelecimento.
                    {% endif %}
                </div>
            </div>
            {% endfor %}
        ...
        <div class="section">
            <h2>5. ANÁLISE ESTATÍSTICA AVANÇADA</h2>
            
            <div class="summary-box">
                <h3>5.1 Indicadores de Variabilidade e Confiabilidade</h3>
                <p><strong>Desvio Padrão dos Picos:</strong> {{ desvio_padrao }} W</p>
                <p><strong>Coeficiente de Variação:</strong> {{ coef_variacao }}% - {{ interpretacao_diversidade }}</p>
                <p><strong>Intervalo de Confiança (95%):</strong> {{ ic_inferior }} W - {{ ic_superior }} W</p>
                <p><strong>Amplitude de Variação:</strong> {{ "%.0f"|format((pico_max|float - pico_min|float)) }} W</p>
            </div>
            
            <h3>5.2 Indicadores de Eficiência Energética</h3>
            <div class="parametros-lista">
                <p><strong>Consumo Médio Diário:</strong> {{ consumo_medio }} kWh</p>
                <p><strong>Fator de Carga Médio:</strong> {{ fator_carga_medio }}% - {{ interpretacao_fator_carga }}</p>
                <p><strong>Fator de Diversidade:</strong> {{ fator_diversidade }}</p>
                <p><strong>Densidade de Carga:</strong> {{ "%.2f"|format(pico_medio|float / total_instancias) }} W/unidade</p>
            </div>
            
            <div class="destaque">
                <h4>Interpretação dos Indicadores</h4>
                <p>O <strong>Coeficiente de Variação</strong> de {{ coef_variacao }}% indica {{ interpretacao_diversidade }}. Valores abaixo de 15% sugerem comportamento altamente previsível, enquanto valores acima de 30% indicam alta variabilidade que requer análise mais detalhada.</p>
                
                <p>O <strong>Fator de Carga</strong> de {{ fator_carga_medio }}% {{ interpretacao_fator_carga }}. Este indicador é crucial para avaliar a eficiência da utilização da infraestrutura instalada e identificar oportunidades de otimização.</p>
                
                <p>O <strong>Fator de Diversidade</strong> de {{ fator_diversidade }} representa a relação entre a demanda máxima observada e a soma das potências instaladas, demonstrando o benefício da não simultaneidade de operação dos equipamentos.</p>
            </div>
        </div>

        <div class="section">
            <h2>6. RECOMENDAÇÕES TÉCNICAS PARA DIMENSIONAMENTO</h2>
            
            <div class="recomendacoes">
                <h3>6.1 Dimensionamento da Infraestrutura Elétrica Principal</h3>
                <ul>
                    <li><strong>Capacidade Recomendada para Transformadores:</strong> {{ capacidade_recomendada }} W (P95 + 20% de margem de segurança)</li>
                    <li><strong>Dimensionamento de Condutores:</strong> Baseado na capacidade recomendada, considerando fatores de correção por temperatura e agrupamento conforme NBR 5410</li>
                    <li><strong>Sistemas de Proteção:</strong> Ajustes baseados no P95 com coordenação seletiva para garantir continuidade do serviço</li>
                    <li><strong>Fator de Demanda Global:</strong> {{ "%.2f"|format(pico_95|float / (pico_medio|float * 1.2)) }} para aplicação em projetos similares</li>
                </ul>
                
                <h3>6.2 Estratégias de Gestão de Demanda</h3>
                <ul>
                    <li><strong>Monitoramento Contínuo:</strong> Implementação de sistema de medição inteligente para validação dos resultados da simulação e identificação de desvios comportamentais</li>
                    <li><strong>Gestão de Cargas:</strong> Desenvolvimento de estratégias de controle automático para equipamentos de maior consumo durante períodos de pico</li>
                    <li><strong>Eficiência Energética:</strong> Priorização de investimentos em equipamentos de maior contribuição para a demanda total, conforme identificado na análise de potência cumulativa</li>
                    <li><strong>Sistemas de Backup:</strong> Dimensionamento de geradores de emergência baseado no P95, com possibilidade de operação seletiva por setores</li>
                </ul>
                
                <h3>6.3 Considerações para Expansões Futuras</h3>
                <ul>
                    <li><strong>Escalabilidade:</strong> A metodologia aplicada permite reavaliação rápida com diferentes configurações de cômodos e equipamentos</li>
                    <li><strong>Margem para Crescimento:</strong> Recomenda-se reserva de 25-30% da capacidade instalada para futuras expansões ou mudanças de perfil de uso</li>
                    <li><strong>Flexibilidade Operacional:</strong> Projeto de infraestrutura modular que permita adequações conforme evolução do comportamento dos usuários</li>
                </ul>
                
                <h3>6.4 Validação e Monitoramento</h3>
                <ul>
                    <li><strong>Período de Validação:</strong> Recomenda-se monitoramento por pelo menos 12 meses para capturar variações sazonais</li>
                    <li><strong>Indicadores de Acompanhamento:</strong> Comparação mensal entre demandas observadas e simuladas, com tolerância de ±15%</li>
                    <li><strong>Revisão Periódica:</strong> Atualização da simulação a cada 2-3 anos ou quando houver mudanças significativas no perfil de ocupação</li>
                </ul>
            </div>
        </div>

        <div class="section">
            <h2>7. CONCLUSÕES E CONSIDERAÇÕES FINAIS</h2>
            
            <p>A simulação Monte Carlo realizada com {{ num_simulacoes }} cenários independentes fornece uma base estatisticamente robusta para o dimensionamento da infraestrutura elétrica do estabelecimento hoteleiro. A metodologia de instâncias individualizadas permite capturar adequadamente o fator de diversidade, resultando em dimensionamentos mais precisos e economicamente otimizados.</p>
            
            <div class="warning-box">
                <h4>Limitações e Premissas do Estudo</h4>
                <p>Os resultados apresentados baseiam-se nas configurações de equipamentos e padrões de uso fornecidos. Mudanças significativas no perfil de ocupação, introdução de novos tipos de equipamentos ou alterações nos hábitos dos usuários podem impactar os resultados e requerem reavaliação da simulação.</p>
            </div>
            
            <p>A capacidade recomendada de {{ capacidade_recomendada }} W (baseada no P95 com margem de segurança) oferece um equilíbrio adequado entre confiabilidade operacional e viabilidade econômica. Esta recomendação considera as melhores práticas da engenharia elétrica e está alinhada com normas técnicas nacionais e internacionais.</p>
            
            <p>Recomenda-se fortemente a implementação de sistemas de monitoramento contínuo para validação dos resultados e refinamento progressivo dos modelos de simulação. Esta abordagem permite otimização contínua da operação e identificação precoce de necessidades de adequação da infraestrutura.</p>
        </div>

        <div class="footer">
            <p><strong>Relatório Técnico Gerado Automaticamente</strong></p>
            <p>Sistema de Simulação Monte Carlo para Análise de Carga Elétrica</p>
            <p>Desenvolvido por <strong>Matheus Vianna</strong> | <a href="https://matheusvianna.com" target="_blank">matheusvianna.com</a></p>
            <p>Análise baseada em {{ num_simulacoes }} simulações independentes | Metodologia validada conforme práticas da engenharia elétrica</p>
            <p>Para questões técnicas ou esclarecimentos adicionais, consulte a documentação técnica do sistema</p>
        </div>
    </body>
    </html>
    """
    ...
    if comodos_originais:
        for comodo_obj in comodos_originais:
            for eq in comodo_obj.equipamentos:
                total_potencia_instalada_comodos += eq.potencia * eq.quantidade * instancias_por_comodo.get(comodo_obj.nome, 1)
    ...
    # Calcula potências por cômodo (estimativa técnica)
    potencias_por_comodo = {}
    for comodo, instancias in instancias_por_comodo.items():
        potencias_por_comodo[comodo] = f"{(pico_medio / sum(instancias_por_comodo.values()) * instancias):.0f}"
    
    # Processa imagens dos gráficos se fornecidas
    imagens_processadas = []
    if imagens_graficos:
        for imagem in imagens_graficos:
            with open(imagem['caminho'], 'rb') as f:
                img_data = f.read()
    ...
    # Prepara dados da configuração dos cômodos para o template
    comodos_config_for_pdf = {}
    if comodos_config_data:
        for comodo_name, df in comodos_config_data.items():
            comodos_config_for_pdf[comodo_name] = df.to_dict(orient='records')
    elif comodos_originais:
        for comodo_obj in comodos_originais:
            equip_data = []
            for eq in comodo_obj.equipamentos:
                equip_data.append({
                    ...
                    'Quantidade': eq.quantidade,
                    'Tipo de intervalo': 'fixo' if not callable(eq.intervalos) else 'dinâmico',
                    'intervalo': str(eq.intervalos) if not callable(eq.intervalos) else 'Dinâmico',
                    'probabilidade': eq.probabilidade,
                    'FD': eq.fator_demanda
                })
            comodos_config_for_pdf[comodo_obj.nome] = equip_data

    # Prepara dados para o template
    template_data = {
        'data_geracao': datetime.now().strftime("%d/%m/%Y às %H:%M"),
        'num_simulacoes': f"{num_simulacoes:,}",
        'tempo_total': tempo_total,
        'tempo_horas': tempo_total // 60,
        'total_comodos': len(instancias_por_comodo),
        'total_instancias': sum(instancias_por_comodo.values()),
        'pico_medio': f"{pico_medio:.0f}",
        'pico_max': f"{pico_max:.0f}",
        'pico_min': f"{pico_min:.0f}",
        'pico_95': f"{pico_95:.0f}",
        'desvio_padrao': f"{desvio_padrao:.0f}",
        'coef_variacao': f"{coef_variacao:.1f}",
        'consumo_medio': f"{consumo_medio:.1f}",
        'fator_carga_medio': f"{fator_carga_medio:.1f}",
        'ic_inferior': f"{ic_inferior:.0f}",
        'ic_superior': f"{ic_superior:.0f}",
        'capacidade_recomendada': f"{capacidade_recomendada:.0f}",
        'fator_diversidade': f"{fator_diversidade:.2f}",
        'instancias_por_comodo': instancias_por_comodo,
        'potencias_por_comodo': potencias_por_comodo,
        'interpretacao_diversidade': interpretacao_diversidade,
        'interpretacao_fator_carga': interpretacao_fator_carga,
        'imagens_graficos': imagens_processadas,
        'comodos_config_data': comodos_config_for_pdf
    }
